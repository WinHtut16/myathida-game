...
> **⚠️ Superseded in part — see "v2 change note" below.**
> After client review, the scope changed: the shop runs each TV's timer and
> on/off through the **CozyLife smart-plug app**, so this system dropped live
> timers, extend/pause, group tabs, deposits, and all hardware control. It is now
> a **cost calculator + session-history recorder** with a manual occupancy board.
> Pricing moved to per-tier (PS4 3,000 / PS5 5,000 / VIP 7,000 MMK/hr, per-minute
> prorated, 30-min minimum) and a two-role auth model (superadmin by email, admin
> by phone) was added. Sections 3–7 below describe the original v1 design; the
> **README** reflects what is actually built. Kept here for history.
