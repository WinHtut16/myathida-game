...
export function EmptyState({ icon: Icon, message, action, className }: EmptyStateProps) {
  return (
    <div className={cx("text-center py-14 bg-surface border border-line rounded-lg", className)}>
      <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-lg bg-line-faint">
        <Icon className="h-7 w-7 text-text-faint" />
      </div>
      <p className="text-sm font-medium text-text-muted">{message}</p>
      {action && (
        <button onClick={action.onClick} className="mt-2 text-sm font-semibold text-accent hover:underline">
          {action.label}
        </button>
      )}
    </div>
